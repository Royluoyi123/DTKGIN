...
class Aggregator(nn.Module):
    ...
    def forward(self, entity_emb, user_emb, latent_emb,
        ...
        disen_weight1 = torch.mm(nn.Softmax(dim=-1)(disen_weight_att),
...
class GraphConv(nn.Module):
    ...
    def __init__(self, channel, n_hops, n_users,n_items,
                 n_factors, n_relations, interact_mat_user,interact_mat_item,
                 ind, node_dropout_rate=0.5, mess_dropout_rate=0.1):
        super(GraphConv, self).__init__()

        self.convs = nn.ModuleList()
        self.interact_mat_user = interact_mat_user
        self.interact_mat_item = interact_mat_item
        ...
        for i in range(n_hops):
            self.convs.append(Aggregator(n_users=n_users,n_items=n_items,n_factors=n_factors))
    ...
    def forward(self, user_emb, entity_emb, latent_emb, edge_index, edge_type,
                interact_mat_user,interact_mat_item, mess_dropout=True, node_dropout=False):

        """node dropout"""
        if node_dropout:
            edge_index, edge_type = self._edge_sampling(edge_index, edge_type, self.node_dropout_rate)
            interact_mat_user = self._sparse_dropout(interact_mat_user, self.node_dropout_rate)
            interact_mat_item = self._sparse_dropout(interact_mat_item, self.node_dropout_rate)
        ...
        #print(entity_res_emb.shape,user_res_emb.shape)
        return entity_res_emb, user_res_emb, cor


class Recommender(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat_user,adj_mat_item):
        ...
        self.adj_mat_user = adj_mat_user
        self.adj_mat_item = adj_mat_item
        self.graph = graph
    ...
    def _init_weight(self):
        ...
        # [n_users, n_entities]
        self.interact_mat_user = self._convert_sp_mat_to_sp_tensor(self.adj_mat_user).to(self.device)
        self.interact_mat_item = self._convert_sp_mat_to_sp_tensor(self.adj_mat_item).to(self.device)
    ...
    def _init_model(self):
        return GraphConv(channel=self.emb_size,
                         ...
                         n_factors=self.n_factors,
                         interact_mat_user=self.interact_mat_user,
                         interact_mat_item=self.interact_mat_item,
                         ind=self.ind,
    ...
    def forward(self, batch=None):
        ...
        entity_gcn_emb, user_gcn_emb, cor = self.gcn(user_emb,
                                                     ...
                                                     self.edge_index,
                                                     self.edge_type,
                                                     self.interact_mat_user,
                                                     self.interact_mat_item,
